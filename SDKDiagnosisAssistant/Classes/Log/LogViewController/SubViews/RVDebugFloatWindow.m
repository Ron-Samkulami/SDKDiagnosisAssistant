...
@implementation RVDebugFloatWindow
    ...
    if (!self.isShowTab) {
        ...
        [UIView animateWithDuration:showDuration animations:^{
            ...
            if (self.frame.origin.x <= kScreenWidth/2) {
                ...
                self.mainBtn.frame = CGRectMake((_titles.count * (self.frameWidth + marginWith)), 0, self.frameWidth, self.frameWidth);
                self.frame = CGRectMake(self.frame.origin.x  - _titles.count * (self.frameWidth + marginWith), self.frame.origin.y, (WIDTH + _titles.count * (self.frameWidth + marginWith)) ,self.frameWidth);
            }
            if (_bgcolor) {
        ...
        [UIView animateWithDuration:showDuration animations:^{
            ...
            if (self.frame.origin.x + self.mainBtn.frame.origin.x <= kScreenWidth/2) {
                ...
                self.frame = CGRectMake(self.frame.origin.x + _titles.count * (self.frameWidth + marginWith), self.frame.origin.y, self.frameWidth ,self.frameWidth);
            }
            self.backgroundColor = [UIColor clearColor];
        }];
        [self performSelector:@selector(changeStatus) withObject:nil afterDelay:statusChangeDuration];
