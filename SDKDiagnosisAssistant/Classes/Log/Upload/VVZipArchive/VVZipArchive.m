...
#include "minizip/vv_mz_compat.h"
#include "minizip/vv_mz_zip.h"
#include <zlib.h>
#include <sys/stat.h>

NSString *const VVZipArchiveErrorDomain = @"VVZipArchiveErrorDomain";

#define CHUNK 16384

int _vv_zipOpenEntry(zipFile entry, NSString *name, const zip_fileinfo *zipfi, int level, NSString *password, BOOL aes);
BOOL _vv_fileIsSymbolicLink(const vv_unz_file_info *fileInfo);
...
@interface NSData(VVZipArchive)
- (NSString *)vv_base64RFC4648 API_AVAILABLE(macos(10.9), ios(7.0), watchos(2.0), tvos(9.0));
- (NSString *)vv_hexString;
@end

@interface NSString (VVZipArchive)
- (NSString *)vv_sanitizedPath;
@end

@interface VVZipArchive ()
- (instancetype)init NS_DESIGNATED_INITIALIZER;
@end

@implementation VVZipArchive
{
    /// path for zip file
    NSString *_path;
    zipFile _zip;
...
+ (BOOL)isFilePasswordProtectedAtPath:(NSString *)path {
    // Begin opening
    zipFile zip = vv_unzOpen(path.fileSystemRepresentation);
    if (zip == NULL) {
        return NO;
    }
    
    BOOL passwordProtected = NO;
    int ret = vv_unzGoToFirstFile(zip);
    if (ret == VV_UNZ_OK) {
        do {
            ret = vv_unzOpenCurrentFile(zip);
            if (ret != VV_UNZ_OK) {
                // attempting with an arbitrary password to workaround `vv_unzOpenCurrentFile` limitation on AES encrypted files
                ret = vv_unzOpenCurrentFilePassword(zip, "");
                vv_unzCloseCurrentFile(zip);
                if (ret == VV_UNZ_OK || ret == VV_MZ_PASSWORD_ERROR) {
                    passwordProtected = YES;
                }
                break;
            }
            vv_unz_file_info fileInfo = {};
            ret = vv_unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            vv_unzCloseCurrentFile(zip);
            if (ret != VV_UNZ_OK) {
                break;
            } else if ((fileInfo.flag & VV_MZ_ZIP_FLAG_ENCRYPTED) == 1) {
                passwordProtected = YES;
                break;
            }
            
            ret = vv_unzGoToNextFile(zip);
        } while (ret == VV_UNZ_OK);
    }
...
+ (BOOL)isPasswordValidForArchiveAtPath:(NSString *)path password:(NSString *)pw error:(NSError **)error {
    ...
    if (zip == NULL) {
        if (error) {
            *error = [NSError errorWithDomain:VVZipArchiveErrorDomain
                                         code:VVZipArchiveErrorCodeFailedOpenZipFile
    ...
    if (ret == VV_UNZ_OK) {
        do {
            if (pw.length == 0) {
                ...
                ret = vv_unzOpenCurrentFilePassword(zip, [pw cStringUsingEncoding:NSUTF8StringEncoding]);
            }
            if (ret != VV_UNZ_OK) {
                if (ret != VV_MZ_PASSWORD_ERROR) {
                    if (error) {
                        *error = [NSError errorWithDomain:VVZipArchiveErrorDomain
                                                     code:VVZipArchiveErrorCodeFailedOpenFileInZip
            ...
            if (ret != VV_UNZ_OK) {
                if (error) {
                    *error = [NSError errorWithDomain:VVZipArchiveErrorDomain
                                                 code:VVZipArchiveErrorCodeFileInfoNotLoadable
                ...
                int readBytes = vv_unzReadCurrentFile(zip, buffer, (unsigned)MIN(10UL,fileInfo.uncompressed_size));
                if (readBytes < 0) {
                    ...
                    if (readBytes != Z_DATA_ERROR) {
                        if (error) {
                            *error = [NSError errorWithDomain:VVZipArchiveErrorDomain
                                                         code:VVZipArchiveErrorCodeFileContentNotReadable
...
{
    return [self vv_unzipFileAtPath:path toDestination:destination delegate:nil];
}

+ (BOOL)vv_unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(nullable NSString *)password error:(NSError **)error
{
    return [self vv_unzipFileAtPath:path toDestination:destination preserveAttributes:YES overwrite:overwrite password:password error:error delegate:nil progressHandler:nil completionHandler:nil];
}
...
{
    return [self vv_unzipFileAtPath:path toDestination:destination preserveAttributes:YES overwrite:YES password:nil error:nil delegate:delegate progressHandler:nil completionHandler:nil];
}
...
{
    return [self vv_unzipFileAtPath:path toDestination:destination preserveAttributes:YES overwrite:overwrite password:password error:error delegate:delegate progressHandler:nil completionHandler:nil];
}
...
+ (BOOL)vv_unzipFileAtPath:(NSString *)path
        ...
        progressHandler:(void (^)(NSString *entry, vv_unz_file_info zipInfo, long entryNumber, long total))progressHandler
      completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
    return [self vv_unzipFileAtPath:path toDestination:destination preserveAttributes:YES overwrite:overwrite password:password error:nil delegate:nil progressHandler:progressHandler completionHandler:completionHandler];
}
...
{
    return [self vv_unzipFileAtPath:path toDestination:destination preserveAttributes:YES overwrite:YES password:nil error:nil delegate:nil progressHandler:progressHandler completionHandler:completionHandler];
}
...
+ (BOOL)vv_unzipFileAtPath:(NSString *)path
     ...
     preserveAttributes:(BOOL)preserveAttributes
              overwrite:(BOOL)overwrite
               ...
               delegate:(nullable id<VVZipArchiveDelegate>)delegate
{
    return [self vv_unzipFileAtPath:path toDestination:destination preserveAttributes:preserveAttributes overwrite:overwrite password:password error:error delegate:delegate progressHandler:nil completionHandler:nil];
}
...
+ (BOOL)vv_unzipFileAtPath:(NSString *)path
     ...
     preserveAttributes:(BOOL)preserveAttributes
      ...
      completionHandler:(void (^_Nullable)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
    return [self vv_unzipFileAtPath:path toDestination:destination preserveAttributes:preserveAttributes overwrite:overwrite nestedZipLevel:0 password:password error:error delegate:delegate progressHandler:progressHandler completionHandler:completionHandler];
}
      ...
      completionHandler:(void (^_Nullable)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
    ...
    if (path.length == 0 || destination.length == 0)
    {
        ...
        NSError *err = [NSError errorWithDomain:VVZipArchiveErrorDomain code:VVZipArchiveErrorCodeInvalidArguments userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (zip == NULL)
    {
        ...
        NSError *err = [NSError errorWithDomain:VVZipArchiveErrorDomain code:VVZipArchiveErrorCodeFailedOpenZipFile userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    vv_unz_global_info globalInfo = {};
    vv_unzGetGlobalInfo(zip, &globalInfo);
    
    // Begin vv_unzipping
    int ret = 0;
    ret = vv_unzGoToFirstFile(zip);
    if (ret != VV_UNZ_OK && ret != VV_MZ_END_OF_LIST)
    {
        NSDictionary *userInfo = @{NSLocalizedDescriptionKey: @"failed to open first file in zip file"};
        NSError *err = [NSError errorWithDomain:VVZipArchiveErrorDomain code:VVZipArchiveErrorCodeFailedOpenFileInZip userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    NSError *vv_unzippingError;
    do {
        currentFileNumber++;
        if (ret == VV_MZ_END_OF_LIST) {
            break;
        }
        @autoreleasepool {
            if (password.length == 0) {
                ...
                ret = vv_unzOpenCurrentFilePassword(zip, [password cStringUsingEncoding:NSUTF8StringEncoding]);
            }
            
            if (ret != VV_UNZ_OK) {
                vv_unzippingError = [NSError errorWithDomain:@"VVZipArchiveErrorDomain" code:VVZipArchiveErrorCodeFailedOpenFileInZip userInfo:@{NSLocalizedDescriptionKey: @"failed to open file in zip file"}];
                success = NO;
                break;
            }
            ...
            vv_unz_file_info fileInfo;
            memset(&fileInfo, 0, sizeof(vv_unz_file_info));
            
            ret = vv_unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            if (ret != VV_UNZ_OK) {
                vv_unzippingError = [NSError errorWithDomain:@"VVZipArchiveErrorDomain" code:VVZipArchiveErrorCodeFileInfoNotLoadable userInfo:@{NSLocalizedDescriptionKey: @"failed to retrieve info for file"}];
                success = NO;
                vv_unzCloseCurrentFile(zip);
                break;
            }
            ...
            vv_unzGetCurrentFileInfo(zip, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
            filename[fileInfo.size_filename] = '\0';
            
            BOOL vv_fileIsSymbolicLink = _vv_fileIsSymbolicLink(&fileInfo);
            
            NSString * strPath = [VVZipArchive _filenameStringWithCString:filename
            ...
            strPath = [strPath vv_sanitizedPath];
            if (!strPath.length) {
            ...
            if (preserveAttributes) {
                NSDate *modDate = [[self class] _dateWithMSDOSFormat:(UInt32)fileInfo.vv_mz_dos_date];
            ...
            if (err != nil) {
                if ([err.domain isEqualToString:NSCocoaErrorDomain] &&
                    err.code == 640) {
                    vv_unzippingError = err;
                    vv_unzCloseCurrentFile(zip);
                    success = NO;
                    break;
                }
                NSLog(@"[VVZipArchive] Error: %@", err.localizedDescription);
            }
            ...
            if (isDirectory && !vv_fileIsSymbolicLink) {
                // nothing to read/write for a directory
            } else if (!vv_fileIsSymbolicLink) {
                // ensure we are not creating stale file entries
                int readBytes = vv_unzReadCurrentFile(zip, buffer, 4096);
                if (readBytes >= 0) {
                    ...
                    while (fp) {
                        if (readBytes > 0) {
                            if (0 == fwrite(buffer, readBytes, 1, fp)) {
                                if (ferror(fp)) {
                                    ...
                                    NSLog(@"[VVZipArchive] %@", message);
                                    success = NO;
                                    vv_unzippingError = [NSError errorWithDomain:@"VVZipArchiveErrorDomain" code:VVZipArchiveErrorCodeFailedToWriteFile userInfo:@{NSLocalizedDescriptionKey: message}];
                                    break;
                                }
                            }
                        } else {
                        ...
                        readBytes = vv_unzReadCurrentFile(zip, buffer, 4096);
                        if (readBytes < 0) {
                            // Let's assume error Z_DATA_ERROR is caused by an invalid password
                            // Let's assume other errors are caused by Content Not Readable
                            success = NO;
                    ...
                    if (fp) {
                        ...
                        if (nestedZipLevel
                            ...
                            && [self vv_unzipFileAtPath:fullPath
                            ...
                            if (fileInfo.vv_mz_dos_date != 0) {
                                NSDate *orgDate = [[self class] _dateWithMSDOSFormat:(UInt32)fileInfo.vv_mz_dos_date];
                                NSDictionary *attr = @{NSFileModificationDate: orgDate};
                                
                                if (attr) {
                                    if (![fileManager setAttributes:attr ofItemAtPath:fullPath error:nil]) {
                                        // Can't set attributes
                                        NSLog(@"[VVZipArchive] Failed to set attributes - whilst setting modification date");
                                    }
                                }
                            }
                            ...
                            if (permissions != 0) {
                                ...
                                if (![fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil]) {
                                    ...
                                    NSLog(@"[VVZipArchive] Failed to set attributes - whilst setting permissions");
                                }
                            }
                        }
                    }
                    else
                    {
                        ...
                        switch (errnoSave) {
                                ...
                                // Just log the error
                            {
                                NSError *errorObject = [NSError errorWithDomain:NSPOSIXErrorDomain
                                                                           code:errnoSave
                                                                       userInfo:nil];
                                NSLog(@"[VVZipArchive] Failed to open file on vv_unzipping.(%@)", errorObject);
                            }
                                break;
                        }
                        ...
                        if (isSeriousError) {
                            // serious case
                            vv_unzippingError = [NSError errorWithDomain:NSPOSIXErrorDomain
                                                                 code:errnoSave
                                                             userInfo:nil];
                            vv_unzCloseCurrentFile(zip);
                            // Log the error
                            NSLog(@"[VVZipArchive] Failed to open file on vv_unzipping.(%@)", vv_unzippingError);
            ...
            else
            {
                ...
                int bytesRead = 0;
                while ((bytesRead = vv_unzReadCurrentFile(zip, buffer, 4096)) > 0)
                {
                    buffer[bytesRead] = 0;
                ...
                if (overwrite)
                {
                    if ([fileManager fileExistsAtPath:fullPath])
                    {
                        ...
                        if (!removeSuccess)
                        {
                            ...
                            NSLog(@"[VVZipArchive] %@", message);
                            success = NO;
                            vv_unzippingError = [NSError errorWithDomain:VVZipArchiveErrorDomain code:localError.code userInfo:@{NSLocalizedDescriptionKey: message}];
                        }
                    }
                }
                ...
                if (symlinkError != 0)
                {
                    ...
                    NSLog(@"[VVZipArchive] %@", message);
                    success = NO;
                    vv_unzippingError = [NSError errorWithDomain:NSPOSIXErrorDomain code:symlinkError userInfo:@{NSLocalizedDescriptionKey: message}];
                }
            }
            
            crc_ret = vv_unzCloseCurrentFile(zip);
            if (crc_ret == VV_MZ_CRC_ERROR) {
                // CRC ERROR
                success = NO;
                break;
            }
            ...
            if ([delegate respondsToSelector:@selector(zipArchiveDidUnzipFileAtIndex:totalFiles:archivePath:fileInfo:)]) {
                [delegate zipArchiveDidUnzipFileAtIndex:currentFileNumber totalFiles:(NSInteger)globalInfo.number_entry
                                            archivePath:path fileInfo:fileInfo];
            } else if ([delegate respondsToSelector: @selector(zipArchiveDidUnzipFileAtIndex:totalFiles:archivePath:vv_unzippedFilePath:)]) {
                [delegate zipArchiveDidUnzipFileAtIndex: currentFileNumber totalFiles: (NSInteger)globalInfo.number_entry
                                            archivePath:path vv_unzippedFilePath: fullPath];
            }
    ...
    if (success && preserveAttributes) {
        ...
        for (NSDictionary * d in directoriesModificationDates) {
            if (![[NSFileManager defaultManager] setAttributes:@{NSFileModificationDate: [d objectForKey:@"modDate"]} ofItemAtPath:[d objectForKey:@"path"] error:&err]) {
                NSLog(@"[VVZipArchive] Set attributes failed for directory: %@.", [d objectForKey:@"path"]);
            }
            if (err) {
                NSLog(@"[VVZipArchive] Error setting directory file modification date attribute: %@", err.localizedDescription);
            }
        }
    }
    
    // Message delegate
    if (success && [delegate respondsToSelector:@selector(zipArchiveDidUnzipArchiveAtPath:zipInfo:vv_unzippedPath:)]) {
        [delegate zipArchiveDidUnzipArchiveAtPath:path zipInfo:globalInfo vv_unzippedPath:destination];
    }
    ...
    if (crc_ret == VV_MZ_CRC_ERROR)
    {
        ...
        retErr = [NSError errorWithDomain:VVZipArchiveErrorDomain code:VVZipArchiveErrorCodeFileInfoNotLoadable userInfo:userInfo];
    }
    
    if (error) {
        if (vv_unzippingError) {
            *error = vv_unzippingError;
        }
        else {
            *error = retErr;
        }
    ...
    {
        if (vv_unzippingError) {
            completionHandler(path, success, vv_unzippingError);
        }
        else
        {
...
{
    return [VVZipArchive createZipFileAtPath:path withFilesAtPaths:paths withPassword:nil];
}
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath {
    return [VVZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory {
    return [VVZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:keepParentDirectory withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray<NSString *> *)paths withPassword:(NSString *)password
{
    VVZipArchive *zipArchive = [[VVZipArchive alloc] initWithPath:path];
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath withPassword:(nullable NSString *)password {
    return [VVZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:NO withPassword:password];
}


+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory withPassword:(nullable NSString *)password {
    return [VVZipArchive createZipFileAtPath:path
...
- (BOOL)open
{
    ...
    _zip = vv_zipOpen(_path.fileSystemRepresentation, APPEND_STATUS_CREATE);
    return (NULL != _zip);
}
...
- (BOOL)writeFolderAtPath:(NSString *)path withFolderName:(NSString *)folderName withPassword:(nullable NSString *)password
{
    ...
    [VVZipArchive zipInfo:&zipInfo setAttributesOfItemAtPath:path];
    
    int error = _vv_zipOpenEntry(_zip, [folderName stringByAppendingString:@"/"], &zipInfo, Z_NO_COMPRESSION, password, NO);
    const void *buffer = NULL;
    vv_zipWriteInFileInZip(_zip, buffer, 0);
    vv_zipCloseFileInZip(_zip);
    return error == ZIP_OK;
}
...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(nullable NSString *)fileName compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    int error = _vv_zipOpenEntry(_zip, fileName, &zipInfo, compressionLevel, password, aes);
    
    while (!feof(input) && !ferror(input))
    {
        unsigned int len = (unsigned int) fread(buffer, 1, CHUNK, input);
        vv_zipWriteInFileInZip(_zip, buffer, len);
    }
...
- (BOOL)writeData:(NSData *)data filename:(nullable NSString *)filename compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    zip_fileinfo zipInfo = {};
    [VVZipArchive zipInfo:&zipInfo setDate:[NSDate date]];
    
    int error = _vv_zipOpenEntry(_zip, filename, &zipInfo, compressionLevel, password, aes);
    
    vv_zipWriteInFileInZip(_zip, data.bytes, (unsigned int)data.length);
...
- (BOOL)close
{
    NSAssert((_zip != NULL), @"[VVZipArchive] Attempting to close an archive which was never opened");
    int error = vv_zipClose(_zip, NULL);
    _zip = nil;
    return error == ZIP_OK;
}
...
{
    NSCalendar *currentCalendar = VVZipArchive._gregorian;
    ...
    tvv_mz_date.tm_sec = (unsigned int)components.second;
    tvv_mz_date.tm_min = (unsigned int)components.minute;
    tvv_mz_date.tm_hour = (unsigned int)components.hour;
    tvv_mz_date.tm_mday = (unsigned int)components.day;
    // ISO/IEC 9899 struct tm is 0-indexed for January but NSDateComponents for gregorianCalendar is 1-indexed for January
    tvv_mz_date.tm_mon = (unsigned int)components.month - 1;
    // ISO/IEC 9899 struct tm is 0-indexed for AD 1900 but NSDateComponents for gregorianCalendar is 1-indexed for AD 1
    tvv_mz_date.tm_year = (unsigned int)components.year - 1900;
    zipInfo->vv_mz_dos_date = vv_mz_zip_tm_to_dosdate(&tvv_mz_date);
}
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime
{
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[VVZipArchive] MSDOS date masks don't add up");
...
int _vv_zipOpenEntry(zipFile entry, NSString *name, const zip_fileinfo *zipfi, int level, NSString *password, BOOL aes)
{
    ...
    return vv_zipOpenNewFileInZip5(entry, name.fileSystemRepresentation, zipfi, NULL, 0, NULL, 0, NULL, Z_DEFLATED, level, 0, -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, password.UTF8String, aes, made_on_darwin, flag_base, 0);
}
...
BOOL _vv_fileIsSymbolicLink(const vv_unz_file_info *fileInfo)
{
    ...
    BOOL vv_fileIsSymbolicLink = ((fileInfo->version >> 8) == ZipUNIXVersion) && BSD_IFLNK == (BSD_SFMT & (fileInfo->external_fa >> 16));
    return vv_fileIsSymbolicLink;
}

#pragma mark - Private tools for unreadable encodings

@implementation NSData (VVZipArchive)
...
@implementation NSString (VVZipArchive)

// One implementation alternative would be to use the algorithm found at vv_mz_path_resolve from https://github.com/nmoinvaz/minizip/blob/dev/vv_mz_os.c,
